...
int Battery::darin(struct request packet)
{
    if(packet.func == "drain")
    {
        battery_darin(packet);
    }
...
int Battery::battery_darin(struct request packet)
{
    int unitTotal = 1;


    brightness = device->findChild<QSlider*>("brightnessSlider");
